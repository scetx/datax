...
def getBestMatches(alg,test_img,top_k):
    ##### Init Params
    data_path=r"C:\Users\tobias.grab\IWK_data\test2"
    ...
    if category=="old but gold":
        ...
        best_matches=pd.DataFrame(np.array(best_matches.sort_values(by='score',ascending="False")[::-1]),columns=["name","score"])
        avg_score=np.mean(nrOfGoodPerImage)
